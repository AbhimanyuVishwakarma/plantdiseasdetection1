...
# Remove MODEL_URLS and download logic
# Instead, check if the model file exists and load it, else print error
missing_models = [name for name, path in MODEL_PATHS.items() if not os.path.exists(path)]
if missing_models:
    print(f"Error: The following fine-tuned model files are missing and will not be used: {', '.join(missing_models)}")
